...
def bfs(n: int, m: int, maps: list[str]) -> int:
    queue = deque([(0, 0, 0)])
    ...
    while queue:
        ...
        for dx, dy in DIRECTIONS:
            ...
            if not (0 <= nx < n and 0 <= ny < m):
                continue
            if maps[nx][ny] == "0" and dist[nx][ny][broken_cnt] == -1:
                dist[nx][ny][broken_cnt] = next_dist
                queue.append((nx, ny, broken_cnt))
            if maps[nx][ny] == "1" and broken_cnt < MAX_BROKEN and dist[nx][ny][broken_cnt + 1] == -1:
...
def solve(n: int, m: int, maps: list[str]) -> int:
    return bfs(n, m, maps)
...
def main() -> None:
    ...
    maps = [sys_input().rstrip() for _ in range(N)]
